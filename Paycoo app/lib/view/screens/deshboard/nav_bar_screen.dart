...
class _NavBarScreenState extends State<NavBarScreen> {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
          ...
          child: GetBuilder<CustomMenuController>(builder: (CustomMenuController) {
            return Scaffold(
              backgroundColor: Theme.of(context).bottomNavigationBarTheme.selectedItemColor,
              body: PageStorage(bucket: bucket, child: CustomMenuController.currentScreen),
              ...
              bottomNavigationBar: Container(height: 60, decoration: BoxDecoration(color: Theme.of(context).bottomNavigationBarTheme.backgroundColor,
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                  children: [
                    customBottomItem(tap: () => CustomMenuController.selectHomePage(),
                      icon: CustomMenuController.currentTab == 0 ? Images.home_icon_bold : Images.home_icon, name: 'home'.tr, selectIndex: 0,),
                    customBottomItem(tap: () => CustomMenuController.selectHistoryPage(),
                      icon: CustomMenuController.currentTab == 1 ? Images.clock_icon_bold : Images.clock_icon, name: 'history'.tr, selectIndex: 1),
                    const SizedBox(height: 20, width: 20),
                    customBottomItem(tap: () => CustomMenuController.selectNotificationPage(),
                      icon: CustomMenuController.currentTab == 2 ? Images.notification_icon_bold : Images.notification_icon, name: 'notification'.tr, selectIndex: 2,
                    ),
                    customBottomItem(tap: () => CustomMenuController.selectProfilePage(),
                      icon: CustomMenuController.currentTab == 3 ? Images.profile_icon_bold : Images.profile_icon, name: 'profile'.tr, selectIndex: 3,
                    ),
                  ],
                ),
              ),
